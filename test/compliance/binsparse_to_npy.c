...
int main(int argc, char** argv) {
  ...
  if (format == NULL)
    die("input header has no format");
  ...
  if (cJSON_IsTrue(fill)) {
    ...
    if (file < 0 ||
        bsp_read_array(&fill_value, file, "fill_value") != BSP_SUCCESS)
      die("cannot read fill_value");
    H5Fclose(file);
    const char* fill_type =
        cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(
            cJSON_GetObjectItemCaseSensitive(header, "data_types"),
            "fill_value"));
  ...
  for (size_t k = 0; k < entries.size; ++k) {
    ...
    for (int logical = 0; logical < rank; ++logical) {
      int stored = 0;
      while (stored < rank && transpose[stored] != (size_t) logical)
        ++stored;
