...
cJSON* read_json(const char* path) {
  ...
  if (!cJSON_IsObject(json))
    die("target header must be a JSON object");
  return json;
}
...
bool header_values_are_iso(cJSON* header) {
  ...
  const char* value =
      cJSON_GetStringValue(cJSON_GetObjectItemCaseSensitive(types, "values"));
...
static size_t stored_dimension(const bsp_tensor_t* tensor, size_t depth) {
  ...
  if (logical >= (size_t) tensor->rank)
    die("transpose is not a permutation");
    ...
    for (size_t p = 0; p < parents; ++p) {
      for (size_t suffix = 0; suffix < width; ++suffix) {
        ...
        memcpy(next + q * (size_t) tensor->rank,
               coords + p * (size_t) tensor->rank, depth * sizeof(*next));
...
entries_t matrix_to_coo(bsp_matrix_t* matrix) {
  ...
  if (matrix->format == BSP_DVEC || matrix->format == BSP_DMAT ||
    ...
    size_t cols =
        result.rank == 1
            ? 1
            : (matrix->format == BSP_DMATC ? matrix->nrows : matrix->ncols);
    for (size_t i = 0; i < matrix->nnz; ++i) {
      size_t coord[2] = {i / cols, i % cols};
      if (result.rank == 1)
        coord[0] = i;
      if (coord[0] >= rows)
        die("dense value count exceeds shape");
      push_entry(&result, coord, matrix->is_iso ? 0 : i);
    }
    return result;
  }
...
bsp_error_t write_predefined(const char* path, bsp_tensor_t tensor,
  ...
  if (matrix.format == BSP_CVEC || matrix.format == BSP_COOR ||
    ...
    if (sparse->rank == 2)
      matrix.indices_1 = sparse->indices[1];
    return bsp_write_matrix(path, matrix, NULL, NULL, compression);
  }
  if (matrix.format == BSP_CSR || matrix.format == BSP_CSC) {
    bsp_dense_t* dense;
    ...
    if (level->kind != BSP_TENSOR_DENSE)
      return BSP_ERROR_INTERNAL;
    dense = level->data;
    if (dense->child->kind != BSP_TENSOR_SPARSE)
      return BSP_ERROR_INTERNAL;
  ...
  if (matrix.format == BSP_DCSR || matrix.format == BSP_DCSC) {
    ...
    if (level->kind != BSP_TENSOR_SPARSE)
      return BSP_ERROR_INTERNAL;
    outer = level->data;
    if (outer->child->kind != BSP_TENSOR_SPARSE)
      return BSP_ERROR_INTERNAL;
...
void apply_transpose(entries_t* entries, const size_t* source,
  ...
  for (size_t k = 0; k < entries->size; ++k) {
    ...
    for (int td = 0; td < entries->rank; ++td) {
      int sd = 0;
      while (sd < entries->rank && source[sd] != target[td])
        ++sd;
      if (sd == entries->rank)
        die("transpose is not a permutation");
...
bsp_array_t values_in_entry_order(const bsp_array_t source,
  ...
  for (size_t i = 0; i < size; ++i)
    memcpy((char*) values.data + i * bsp_type_size(values.type),
           (char*) source.data +
               entries->entry[iso ? 0 : i].value * bsp_type_size(source.type),
...
static size_t prefix_equal(const entry_t* a, const entry_t* b, size_t depth,
  ...
  for (int d = 0; d < rank; ++d)
    if (a->coord[depth + (size_t) d] != b->coord[depth + (size_t) d])
      return 0;
  return 1;
}
...
bsp_level_t* build_level(const reformat_context_t* context, cJSON* description,
  ...
  if (strcmp(name ? name : "", "sparse") == 0) {
    ...
    if (sparse->indices == NULL || child_ptr == NULL)
      die("out of memory");
    for (int d = 0; d < rank; ++d) {
      char key[64];
      ...
      if (bsp_construct_array_t(
              ...
              requested_type(context, key,
                             bsp_pick_integer_type(
                                 dims[depth + (size_t) d]))) != BSP_SUCCESS)
        die("out of memory");
    }
    if (!root) {
      char key[64];
      ...
      if (sparse->pointers_to == NULL ||
          bsp_construct_array_t(
              sparse->pointers_to, parents + 1,
              requested_type(context, key, bsp_pick_integer_type(groups))) !=
              BSP_SUCCESS)
        die("out of memory");
    }
    groups = 0;
    ...
    sparse->child =
        build_level(context, child_desc, entries, dims, depth + (size_t) rank,
  ...
  if (strcmp(name ? name : "", "dense") == 0) {
    ...
    dense->child =
        build_level(context, child_desc, entries, dims, depth + (size_t) rank,
                    child_ptr, children, false, values);
    free(child_ptr);
...
cJSON* predefined_level(const char* format, size_t* transpose, int rank) {
  ...
  if (!strcmp(format, "DVEC"))
    json = "{\"level_desc\":\"dense\",\"rank\":1,\"level\":{\"level_desc\":"
           "\"element\"}}";
  else if (!strcmp(format, "CVEC"))
    json = "{\"level_desc\":\"sparse\",\"rank\":1,\"level\":{\"level_desc\":"
           "\"element\"}}";
  else if (!strcmp(format, "DMAT") || !strcmp(format, "DMATR"))
    json = "{\"level_desc\":\"dense\",\"rank\":2,\"level\":{\"level_desc\":"
           "\"element\"}}";
  else if (!strcmp(format, "DMATC")) {
    json = "{\"level_desc\":\"dense\",\"rank\":2,\"level\":{\"level_desc\":"
           "\"element\"}}";
    transpose[0] = 1;
    transpose[1] = 0;
  } else if (!strcmp(format, "COO") || !strcmp(format, "COOR"))
    json = "{\"level_desc\":\"sparse\",\"rank\":2,\"level\":{\"level_desc\":"
           "\"element\"}}";
  else if (!strcmp(format, "COOC")) {
    json = "{\"level_desc\":\"sparse\",\"rank\":2,\"level\":{\"level_desc\":"
           "\"element\"}}";
    transpose[0] = 1;
    transpose[1] = 0;
  } else if (!strcmp(format, "CSR"))
    json = "{\"level_desc\":\"dense\",\"rank\":1,\"level\":{\"level_desc\":"
           "\"sparse\",\"rank\":1,\"level\":{\"level_desc\":\"element\"}}}";
  else if (!strcmp(format, "CSC")) {
    json = "{\"level_desc\":\"dense\",\"rank\":1,\"level\":{\"level_desc\":"
           "\"sparse\",\"rank\":1,\"level\":{\"level_desc\":\"element\"}}}";
    transpose[0] = 1;
    transpose[1] = 0;
  } else if (!strcmp(format, "DCSR"))
    json = "{\"level_desc\":\"sparse\",\"rank\":1,\"level\":{\"level_desc\":"
           "\"sparse\",\"rank\":1,\"level\":{\"level_desc\":\"element\"}}}";
  else if (!strcmp(format, "DCSC")) {
    json = "{\"level_desc\":\"sparse\",\"rank\":1,\"level\":{\"level_desc\":"
           "\"sparse\",\"rank\":1,\"level\":{\"level_desc\":\"element\"}}}";
    transpose[0] = 1;
    transpose[1] = 0;
  }
  if (json == NULL || (rank != 1 && rank != 2))
    return NULL;
...
bsp_error_t bsp_reformat_file(const char* input, const char* output,
  ...
  target_format = cJSON_GetStringValue(
      cJSON_GetObjectItemCaseSensitive(target_header, "format"));
  if (source_format == NULL || target_format == NULL)
    die("header has no format string");
  ...
  if (source_is_tensor) {
    ...
    if (tensor.level == NULL)
      die("failed to parse input tensor");
  ...
  if (!source_transpose || !target_transpose || !stored_dims)
    die("out of memory");
  for (int d = 0; d < rank; ++d) {
    source_transpose[d] =
        source_is_tensor && tensor.transpose ? tensor.transpose[d] : (size_t) d;
  ...
  if (!strcmp(target_format, "custom")) {
    ...
    if (transpose != NULL) {
      if (cJSON_GetArraySize(transpose) != rank)
        die("transpose rank mismatch");
      for (int d = 0; d < rank; ++d)
        target_transpose[d] =
  ...
  for (int d = 0; d < rank; ++d) {
    ...
    stored_dims[d] = source_is_tensor
                         ? tensor.dims[logical]
  ...
  if (!result.dims || !result.transpose)
    die("out of memory");
  for (int d = 0; d < rank; ++d) {
    result.dims[d] = source_is_tensor ? tensor.dims[d]
                                      : (d == 0 ? matrix.nrows : matrix.ncols);
    result.transpose[d] = target_transpose[d];
  }
  root_ptr[0] = 0;
  root_ptr[1] = entries.size;
